...
describe("Messages component", () => {
  ...
  test("should render messages", () => {
    ...
    expect(screen.getAllByTestId("msg-list-item")).toHaveLength(
      messages.length
    );
  });

  test("should render unique messages", () => {
  ...
  test("should render attachment icon for messages with file", () => {
    render(<Messages messages={messages} />);
    const attachmentIcons = screen.getAllByRole("fileIcon");
    expect(attachmentIcons).toHaveLength(1);
  });

  test("should apply different styles for own and other user messages", () => {
    render(<Messages messages={messages} />);
